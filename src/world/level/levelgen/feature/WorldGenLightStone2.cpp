...
WorldGenLightStone2::WorldGenLightStone2()
{
}

bool WorldGenLightStone2::generate(Level &level, Random &random, int_t x, int_t y, int_t z)
{
	// Alpha: WorldGenLightStone2.java:14-16 - anchor must be air
	if (level.getTile(x, y, z) != 0)
	{
		return false;
	}

	// Alpha: WorldGenLightStone2.java:17-19 - and hang from netherrack
	if (level.getTile(x, y + 1, z) != Tile::hellRock.id)
	{
		return false;
	}

	// Alpha: WorldGenLightStone2.java:20 - seed block of the cluster
	level.setTile(x, y, z, Tile::lightGem.id);

	// Alpha: WorldGenLightStone2.java:21-52 - grow the cluster downwards
	for (int_t i = 0; i < 1500; ++i)
	{
		...
		// Alpha: WorldGenLightStone2.java:26-49 - count adjacent glowstone
		int_t neighbors = 0;
		...
		// Alpha: WorldGenLightStone2.java:50 - attach only to a single existing block
		if (neighbors != 1)
		{
			continue;
		}
	...
	// Alpha: WorldGenLightStone2.java:53
	return true;
}
