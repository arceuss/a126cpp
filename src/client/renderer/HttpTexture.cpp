...
void HttpTexture::downloadThread(const jstring &url, HttpTextureProcessor *processor)
{
	try
	{
		...
		if (!fromCache)
		{
			...
			// Download image using WinHttpClient
			std::wstring wurl(url.begin(), url.end());
			WinHttpClient client(L"Alpha126Cpp Skin Downloader");
			std::string responseBody;
			if (client.Get(wurl, responseBody)) {
				// Load image from memory
				image = BufferedImage::ImageIO_readFromMemory(
					reinterpret_cast<const unsigned char*>(responseBody.data()),
					static_cast<int_t>(responseBody.size())
				);
			} else {
				std::cerr << "HTTP request failed for skin: " << urlStr << " (WinHTTP error: " << client.GetLastError() << ")" << std::endl;
			}
		}
