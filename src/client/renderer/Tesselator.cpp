...
{
	// Do not touch GL/SDL here - this constructor may run during static
	// initialization before SDL is initialized. Defer GL setup to
	// ensureGLInitialized().
	this->size = size;
	...
	// Default: VBO disabled until we query capabilities at runtime.
	vboMode = false;
	// vboIds will be created in ensureGLInitialized() if supported.
}

void Tesselator::ensureGLInitialized()
{
	if (glInitialized)
		return;
...
void Tesselator::end()
{
	...
	// Ensure GL context and capabilities are available before issuing GL calls.
	ensureGLInitialized();
