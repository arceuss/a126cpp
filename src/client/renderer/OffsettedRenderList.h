...
private:
	int_t x = 0, y = 0, z = 0;
	std::vector<int_t> lists; // Legacy display lists (for non-chunk rendering like sky/star/dark)
...
public:
	...
	void add(int_t list);  // Legacy display list (for non-chunk rendering)
	void addChunk(std::shared_ptr<Chunk> chunk, int_t layer);  // Add chunk VBO for rendering
