			...
			for (int_t yy = 0; yy < SD; yy++)
			{
				for (int_t zz = 0; zz < SD; zz++)
				{
					...
					auto particle = std::make_unique<TerrainParticle>(
						*level, xp, yp, zp, xp - x - 0.5, yp - y - 0.5, zp - z - 0.5, tile);
					particle->init(x, y, z);
...
void ParticleEngine::crack(int_t x, int_t y, int_t z, int_t face)
{
	...
	if (tid != 0 && Tile::tiles[tid] != nullptr)
	{
		...
		// Beta: Use tile.tex for particles (ParticleEngine.java:164)
		auto particle = std::make_unique<TerrainParticle>(*level, xp, yp, zp, 0.0, 0.0, 0.0, tile);
		particle->init(x, y, z);
...
jstring ParticleEngine::countParticles()
{
	...
	int_t count = particles[0].size() + particles[1].size() + particles[2].size();
