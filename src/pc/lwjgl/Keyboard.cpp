...
// SDL1.2 COMPAT START
// SDL 1.2 key symbols differ from SDL2. Provide compatibility aliases so the
// rest of the code can keep using SDL2-style names.
#include <SDL_keysym.h>

// Keypad naming differences (SDL2: SDLK_KP_0..9, SDL1.2: SDLK_KP0..9)
#ifndef SDLK_KP_0
...
#ifndef SDLK_KP_DECIMAL
#define SDLK_KP_DECIMAL SDLK_KP_PERIOD
#endif

// PageUp legacy name (SDL2 exposes SDLK_PRIOR, SDL1.2 uses SDLK_PAGEUP)
#ifndef SDLK_PRIOR
#define SDLK_PRIOR SDLK_PAGEUP
#endif

// Keys that do not exist in SDL 1.2: give them UNIQUE dummy values so switch-case
// statements don't end up with duplicate case labels (e.g. multiple SDLK_UNKNOWN).
#ifndef SDLK_SLEEP
#define SDLK_SLEEP ((SDLKey)0x4000)
#endif
#ifndef SDLK_STOP
#define SDLK_STOP  ((SDLKey)0x4001)
#endif
#ifndef SDLK_KP_COMMA
#define SDLK_KP_COMMA ((SDLKey)0x4002)
#endif
#ifndef SDLK_F16
#define SDLK_F16 ((SDLKey)0x4010)
#define SDLK_F17 ((SDLKey)0x4011)
#define SDLK_F18 ((SDLKey)0x4012)
#define SDLK_F19 ((SDLKey)0x4013)
#endif
// SDL1.2 COMPAT END
...
{
	static bool down_state[SDLK_LAST] = {false};

	if (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP)
	{
		const int_t sdl_sym = e.key.keysym.sym;
		const bool down = (e.key.state == SDL_PRESSED);
		const bool repeat = down && sdl_sym >= 0 && sdl_sym < SDLK_LAST && down_state[sdl_sym];
		if (sdl_sym >= 0 && sdl_sym < SDLK_LAST)
			down_state[sdl_sym] = down;

		handleKey(keySDLToLWJGL(sdl_sym), repeat, down);

		// SDL 1.2 uses keysym.unicode for text input when Unicode is enabled.
		if (down)
		{
			Uint16 u = e.key.keysym.unicode;
			if (u != 0)
				handleCharacter(static_cast<int_t>(u));
		}
	}
}
...
{
	return String::fromUTF8(SDL_GetKeyName((SDLKey)(keyLWJGLToSDL(key))));
}
...
void enableRepeatEvents(bool repeat)
{
	...
	// SDL 1.2 key repeat is configured globally.
	if (repeat)
...
bool isKeyDown(int_t key)
{
	...
	{
		keyboard_state = SDL_GetKeyState(nullptr);
		if (keyboard_state == nullptr)
			throw SDLException();
	}
	...
	if (sdl_key == SDLK_UNKNOWN)
		return false;
	if (sdl_key < 0 || sdl_key >= SDLK_LAST)
		return false;
	return keyboard_state[sdl_key] != 0;
}
