...
static bool connectTo(const Url &url, int_t timeoutSeconds, SocketHandle &out,
	std::string &error)
{
	...
	if (::getaddrinfo(url.host.c_str(), service.c_str(), &hints, &candidates) != 0 ||
		candidates == nullptr)
	{
	...
	if (handle == INVALID_SOCKET_HANDLE)
	{
		error = "connect failed to " + url.host;
		return false;
	}
...
Response fetch(const std::string &url, int_t timeoutSeconds)
{
	Response response;

#ifdef __SWITCH__
	// libnx's BSD sockets abort instead of returning an error when the service
	// did not initialise, so never reach them without a network.
	...
	SocketHandle handle = INVALID_SOCKET_HANDLE;
	if (!connectTo(parsed, timeoutSeconds, handle, response.error))
		return response;
